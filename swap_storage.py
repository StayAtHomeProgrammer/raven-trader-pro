...
class SwapStorage:
    ...
    def load_swaps(self):
        if not os.path.isfile(SWAP_STORAGE_PATH):
            return []
        with open(SWAP_STORAGE_PATH, mode="r") as fSwap:
            swapJson = fSwap.read()
        try:
            self.swaps = json.loads(swapJson, object_hook=SwapTransaction) # Returns a SwapTransaction class with a __dict__ attribute
        except json.decoder.JSONDecodeError:
            print("Swaps file corrupted or empty")
            return []
    ...
    def load_locked(self):
        if not os.path.isfile(LOCK_STORAGE_PATH):
            return []
        with open(LOCK_STORAGE_PATH, mode="r") as fLock:
            lockJson = fLock.read()
        try:
            self.locks = json.loads(lockJson)
        except json.decoder.JSONDecodeError:
            print("Locks file corrupted or empty")
            return []
        ...
        for asset in self.my_asset_names:
            for a_utxo in self.assets[asset]["outpoints"]:
                if txid == a_utxo["txid"] and vout == a_utxo["vout"]:
                    self.locks.append(
                        {"txid": txid, "vout": vout, "type": "asset", "asset": asset, "amount": a_utxo["amount"]})
            ...
            if matching_asset:
                ...
                for asset_utxo in matching_asset["outpoints"]:
                    if (self.is_taken(asset_utxo, skip_locks)):
                        continue
                    if (float(asset_utxo["amount"]) == float(quantity) and exact) or (
                            asset_utxo["amount"] >= quantity and not exact):
                        return asset_utxo
        return None
    ...
    def find_utxo_set(self, type, quantity, mode="combine", name=None, skip_locks=False):
        ...
        if mode == "combine":
            ...
            quick_check = self.find_utxo(type, quantity, name=name, skip_locks=skip_locks, exact=False, sort_utxo=True)
            quick_check_2 = self.find_utxo(type, quantity, name=name, skip_locks=skip_locks, exact=False,
                                           skip_rounded=False, sort_utxo=True)
            if quick_check:
                # If we have a single UTXO big enough, just use it and get change. sort_utxo ensures we find the smallest first
                found_set = [quick_check]
                total = quick_check["amount"]
