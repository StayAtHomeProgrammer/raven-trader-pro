...
class NewOrderDialog(QDialog):
    ...
    def update(self):
        ...
        if self.mode == "buy":
            ...
            if self.asset_name not in self.swap_storage.my_asset_names or self.quantity > \
                    self.swap_storage.assets[self.asset_name]["balance"]:
                self.valid_order = False
...
class OrderException(Exception):
    # General order exception base class
